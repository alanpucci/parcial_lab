		...
		do
		{
					...
					"\nERROR! Elija una opcion valida", &option, 2, 1, 6)==0)
			{
				...
				case 4:
					...
					{
						printf("\nNo hay cliente con avisos activos cargados\n");
					}
				break;
				case 5:
					...
					{
						printf("\nNo hay cliente con avisos pausados cargados\n");
					}
				break;
				}
			}
...
int info_clientWithMaxActiveAd(Client *clientList, int clientLen, Advertisement *adList, int adLen)
{
	...
	Client bufferMax;
	Client anotherBufferMax;
	if(clientList!=NULL && clientLen>0 && adList!=NULL && adLen>0 && advertisement_searchForActive(adList, adLen)==0)
	{
		for(int i=0;i<clientLen;i++)
		{
		...
		printf("\nEl/Los clientes con mas avisos es/son: ");
		printf("\n%s %s, CUIT: %s cantidad de avisos activos: %d\n", bufferMax.name, bufferMax.lastName, bufferMax.cuit, maxCounter);
		for(int j=0; j<clientLen; j++)
		{
...
int info_clientWithMaxPausedAd(Client *clientList, int clientLen, Advertisement *adList, int adLen)
{
	...
	Client bufferMax;
	Client anotherBufferMax;
	if(clientList!=NULL && clientLen>0 && adList!=NULL && adLen>0 && advertisement_searchForPaused(adList, adLen)==0)
	{
		for(int i=0;i<clientLen;i++)
		{
			if(advertisement_pausedAdsQtyPerClient(adList, adLen, clientList[i].id, &currentCounter) == 0 && (i==0 || currentCounter>maxCounter))
			{
				maxCounter = currentCounter;
				bufferMax = clientList[i];
			}
		...
		printf("\nEl/Los clientes con mas avisos es/son: ");
		printf("\n%s %s, CUIT: %s cantidad de avisos pausados: %d\n", bufferMax.name, bufferMax.lastName, bufferMax.cuit, maxCounter);
		for(int j=0; j<clientLen; j++)
		{
